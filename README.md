...
The two are complementary and both SKILL.md files include a mandatory routing test: an item with a changing status, an owner, a deadline, or a hierarchy belongs in a tracker; everything else is a registry. **The default is always registry** — upgrading later is easy, shrinking a bloated tracker is not.
...
cp -r mercury-skills/registry mercury-skills/tracker <project>/.mercury/spaces/<space>/.pi/skills/
```
...
```bash
cp -r mercury-skills/registry mercury-skills/tracker <project>/.mercury/global/skills/
```
